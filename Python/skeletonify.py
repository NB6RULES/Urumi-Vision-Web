...
def png_to_skeleton_svg(input_path: str, output_path: str):
    import cv2
    import svgwrite
    ...
    dwg = svgwrite.Drawing(output_path, size=(f"{w * mm_per_px}mm", f"{h * mm_per_px}mm"))
    dwg.viewbox(0, 0, w, h)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert extracted PNG to SVG (skeleton/polyline mode)")
    parser.add_argument("-i", "--input", type=str, required=True, help="Input PNG file (with DPI embedded)")
    parser.add_argument("-o", "--output", type=str, default="", help="Output SVG file (default: <input>_skeleton.svg)")
    args = parser.parse_args()

    output = args.output
    if not output:
        output = os.path.splitext(args.input)[0] + "_skeleton.svg"

    png_to_skeleton_svg(args.input, output)
